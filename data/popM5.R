# 5x5 male observed population in wide format
popM5 <- local({
    e <- new.env()
    ...
    as.data.frame(data.table::dcast(e$popAge5dt, country_order + country_code + name + agecat + age ~ year, value.var = "popM")[, `:=`(country_order = NULL, agecat = NULL)][])
})
