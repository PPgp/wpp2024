...
# 5-year age-specific pop estimates (not needed anymore - created on the fly)
...
#pop1[, `:=`(pop_low = popM_low + popF_low, pop_high = pop_high + popF_high)]
...
pop <- popprojAge1dt[, .(popM = sum(popM), popF = sum(popF)), by = c("country_code", "year")]
