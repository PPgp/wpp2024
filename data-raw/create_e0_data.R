...
# 5-year estimates
e0M <- data.table::fread('e0M5.txt', sep = "\t")[, last.observed := NULL][, country := NULL]
e0F <- data.table::fread('e0F5.txt', sep = "\t")[, last.observed := NULL][, country := NULL]
e0B <- data.table::fread('e0B5.txt', sep = "\t")[, last.observed := NULL][, country := NULL]

e0M <- merge.with.countries(e0M)
e0 <- melt.and.merge(NULL, e0M, "e0M", ids = c("country_order", "country_code"))
e0 <- melt.and.merge(e0, e0F, "e0F")
e0 <- melt.and.merge(e0, e0B, "e0B")
setnames(e0, "year", "period")
e0 <- assign.midyear(e0)

## temporarily before e0B is obtained; get the average annual e0B
#yeardf <- data.table::data.table(year = 1951:2023, yearcat = c(rep(seq(1953, 2018, by = 5), each = 5), rep(2023, 3)))
#yeardf[, period := paste(yearcat-3, yearcat + 2, sep = "-")]
#e01 <- merge(e01dt, yeardf, by = "year", sort = FALSE)
#e05 <- e01[, .(e0B = mean(e0B)), by = c("country_code", "yearcat", "period")]
...
setcolorder(e05dt, c("country_code", "year", "period"))
...
e0F95u <- data.table::fread('e0Fproj95u5.txt', sep = "\t")[, country := NULL]
e0M80l <- data.table::fread('e0Mproj80l5.txt', sep = "\t")[, country := NULL]
e0M80u <- data.table::fread('e0Mproj80u5.txt', sep = "\t")[, country := NULL]
e0M95l <- data.table::fread('e0Mproj95l5.txt', sep = "\t")[, country := NULL]
e0M95u <- data.table::fread('e0Mproj95u5.txt', sep = "\t")[, country := NULL]
e0B80l <- data.table::fread('e0Bproj80l5.txt', sep = "\t")[, country := NULL]
e0B80u <- data.table::fread('e0Bproj80u5.txt', sep = "\t")[, country := NULL]
e0B95l <- data.table::fread('e0Bproj95l5.txt', sep = "\t")[, country := NULL]
e0B95u <- data.table::fread('e0Bproj95u5.txt', sep = "\t")[, country := NULL]
...
e0 <- melt.and.merge(e0, e0B80l, "e0B_80l")
e0 <- melt.and.merge(e0, e0B80u, "e0B_80u")
e0 <- melt.and.merge(e0, e0B95l, "e0B_95l")
e0 <- melt.and.merge(e0, e0B95u, "e0B_95u")
setnames(e0, "year", "period")
e0 <- assign.midyear(e0)
